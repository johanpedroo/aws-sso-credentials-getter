  ...
  return readdirSync(path.resolve(homedir(), "./.aws/sso/cache"))
    ...
    .reduce((token, file) => {
      ...
      const data = JSON.parse(
        readFileSync(path.resolve(homedir(), "./.aws/sso/cache", file), "utf-8")
      );
...
const getCredentials = async (sso, params) => {
  return await sso.send(new GetRoleCredentialsCommand(params));
};
...
export default async (profile, customProfile) => {
  ...
  const sso = new SSOClient({ region: sso_region });
